...
from src.task.services import process_task_for_account
...
from src.account.models import TGAccount
from src.task.task_queue import enqueue_task
from pymongo import MongoClient

jobstores = {
    'default': MongoDBJobStore(database='task_manager_db', collection='scheduler_jobs', client=MongoClient('mongodb://localhost:27017'))
}
...
async def schedule_tasks():
    accounts = await TGAccount.find(TGAccount.is_active == True).to_list()
    task = await Task.find_one(Task.status == TaskStatus.pending)

    if not task:
        return

    for account in accounts:
        enqueue_task(task.task_type, account, task)
